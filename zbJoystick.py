...
#ZB.i2c_address = 0x44                  # Uncomment and change the value if you have changed the board address
ZB.init()
if not ZB.foundChip:
...
for i in range(5):
    ...
    if failsafe:
        break
if not failsafe:
    print 'Board %02X failed to report in failsafe mode!' % (ZB.i2c_address)
    sys.exit()
ZB.reset_epo()
    ...
    while running:
        ...
        for event in events:
            ...
            if hadEvent:
                ...
                ZB.set_motor1(-driveLeft * maxPower)
                ZB.set_motor2(-driveLeft * maxPower)
                ZB.set_motor3(driveRight * maxPower)
                ZB.set_motor4(driveRight * maxPower)
